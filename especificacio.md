...
- **Liquidacions directes** (cas no cobert pel mecanisme anterior): algunes operacions de targeta —típicament retirades d'efectiu en caixer— **no entren a la liquidació mensual**: es cobren directament al compte corrent com un càrrec propi, mentre l'extracte de la targeta les registra igualment com un moviment. Sense tractament, la despesa es duplicaria (un cop a cada extracte) i, com que la liquidació mensual mai les inclou, la suma esperada de la quadratura (punt 5) no coincidiria mai amb l'import liquidat. Mecanisme:
  1. Un moviment de targeta es pot marcar com a **liquidació directa** — automàticament per regles configurables sobre el seu propi concepte (predefinides: «RETIRADA EFECTIVO», «DISPOSICION», «CAJERO», «REINTEGRO») amb confirmació explícita, o manualment des del llistat de moviments (marcar/desmarcar qualsevol moviment de targeta).
  2. Un cop marcat, es proposa **aparellar-lo** amb el càrrec corresponent del compte corrent associat (`compteLiquidacioId`): mateix import, dins d'una finestra de ±2 dies. Es confirma manualment (com la resta de suggeriments); si no es troba candidat automàticament, es pot aparellar a mà triant el moviment del compte corrent.
  3. En aparellar-los: el moviment de targeta es marca com a **transferència interna** (queda exclòs dels agregats d'ingressos/despeses i de la detecció de recurrents, igual que a 3.4) i el càrrec del compte corrent —que és qui compta la despesa real, una sola vegada— rep la categoria per defecte **«Efectiu retirat»** (editable com qualsevol altra categoria).
  4. L'aparellament és **idempotent** (tornar a aparellar el mateix parell només actualitza l'estat, no duplica res) i es desfà automàticament si es desfà el lot d'importació d'algun dels dos moviments: el moviment que sobreviu recupera l'estat previ (si és el de targeta, torna a comptar com a despesa normal fins que es torni a aparellar).
  5. El deute de la targeta (suma dels seus moviments, 3.2.1) i la quadratura de la liquidació mensual **exclouen** els moviments marcats com a liquidació directa, ja que mai formaran part d'un càrrec de liquidació.
