...
- **Des del 2026-07-14, cada importació de compromisos (3.2) substitueix tots els `origen='importat'` d'aquest mateix compte** (elimina els anteriors i insereix els del fitxer nou), en lloc de ser additiva amb dedup contra importacions prèvies — vegeu l'entrada d'historial corresponent. No hi ha manera de desfer aquest reemplaçament (no hi ha "lot" per a compromisos, vegeu bullet anterior); si cal recuperar un compromís eliminat per error caldria restaurar des d'una còpia de seguretat (`backupDbFile` es crida abans de qualsevol canvi).
...
### 2026-07-23 — Correcció: ordre de moviments del mateix dia (compte corrent) no coincidia amb el saldo mostrat

Bug reportat per l'usuari: a la taula de Moviments, dos moviments del mateix dia d'un compte corrent (ING, 23/07/2026) es mostraven en l'ordre invers al que reflectia el seu propi saldo. La taula desempatava moviments del mateix dia amb `seq` cru (ordre d'inserció = ordre de fila al fitxer d'origen), assumint implícitament que `seq` ascendent és sempre cronològicament ascendent — cert per a alguns extractes, però l'ING d'aquest compte llista les files de més recent a més antiga, així que dins d'un mateix dia `seq` ascendent és l'ordre cronològic *invers*. Aquest mateix problema ja s'havia detectat i corregit per a comptes de targeta (`ordenaMovimentsTargeta`/`inferDireccio`, vegeu entrades d'historial anteriors "ING-TG-JN"/"ING-TG-JA"), però la solució no s'havia estès a comptes corrent.

A diferència de targeta, un compte corrent porta `saldoPosteriorCents` de banc a cada moviment, un senyal directe més fiable que inferir la direcció del fitxer: `frontend/src/lib/balance.ts` ja reconstruïa l'ordre cronològic real d'un compte corrent encadenant aquest saldo (`ordenaCronologicament`, usada per `saldoEnData`/`creaConsultaSaldo`), però aquesta reconstrucció no s'exposava per a l'ordenació visual de la taula. Nova funció exportada `creaRangCronologicCorrentPerMoviment` (mateix patró que `creaRangCronologicPerMoviment` de targeta, però basada en `ordenaCronologicament` en lloc de `ordenaMovimentsTargeta`); `ordenaCronologicament` s'ha fet genèrica (`<T extends MovimentPerSaldo>`) perquè conservi el camp `id` necessari per indexar el mapa de rangs. A `MovimentsList.tsx`, l'antic `rangCronologicTargetaPerMoviment` (només targeta) s'ha unificat en `rangCronologicPerMoviment`, calculat per a tots els comptes seleccionats (targeta amb `creaRangCronologicPerMoviment`, corrent amb la nova funció), i `clauOrdre` ara el consulta sempre abans de recórrer a `m.seq`.

Verificat amb les dades reals del bug (recurrent seq 1208/1209 del compte ING-CC, 23/07/2026): reconstruint la cadena de saldo, el moviment "Pago en DOYOUSPAIN.COM" (seq 1209, saldo 835,90€) és cronològicament anterior al "Bizum enviado" (seq 1208, saldo 755,90€) — l'invers del que `seq` cru suggeria. 2 tests nous a `balance.test.ts` amb aquests valors reals i un test de consistència amb `creaConsultaSaldo` — 43 tests frontend en total (abans 41), 235 backend sense canvis (no toca backend). `tsc -b`/`oxlint`/`vite build` nets. Pendent confirmació visual de l'usuari.
...
**Backend**: nova funció `editaOcurrenciaPrevista(recurrentId, dataOcurrencia, dades: DadesRecurrent)` a `operations.ts`, amb el mateix mecanisme que `eliminaOcurrenciaPrevista` (avança `data_prevista` del recurrent original a la propera repetició amb `avancaPeriodicitat`), però en lloc de descartar l'ocurrència la converteix en un nou recurrent `unica` amb les dades editades (via `inserirRecurrent`). Llança error si el recurrent no existeix o si és `unica` (no té sentit "només aquesta ocurrència" quan l'única ocurrència ÉS la sèrie — cal editar-lo amb `actualitzaRecurrent`). Nova ruta `POST /api/recurrents/:id/edita-ocurrencia` amb body `{ data, dades }`. Es fa `backupDbFile()` abans (mateixa convenció que `eliminaOcurrenciaPrevista`, ja que avançar `data_prevista` no es pot desfer).

**Frontend**: `frontend/src/lib/recurrentEdit.ts` guanya `esborranyADadesRecurrent` (converteix l'esborrany a `DadesRecurrent`, amb `undefined` en lloc de `null` per als camps opcionals). A `Previsio.tsx`, el formulari d'edició inline mostra ara dos botons quan el recurrent no és `unica` — "Desa només aquesta ocurrència" (crida `editaOcurrenciaPrevista`) i "Desa tota la sèrie" (crida `actualitzaRecurrent`, comportament de sempre) —, i un únic "Desa" quan és `unica` (ja no té sentit la tria).

3 tests nous a `operations.test.ts` (separa una ocurrència periòdica en un nou `unica` sense tocar les futures, llança error amb un recurrent `unica`, llança error amb un id inexistent) — 235 tests backend en total (abans 232), 36 frontend sense canvis. `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes. Verificat contra una còpia temporal de `dades/finances.db` (amb WAL): sobre el recurrent anual real "Assegurança Duster" (2026-07-31, -400€), `POST .../edita-ocurrencia` amb un import i concepte diferents ha creat un nou recurrent `unica` amb aquestes dades a la mateixa data, mentre l'original ha avançat correctament a 2027-07-31 sense canviar l'import; els dos camins d'error (`unica`, id inexistent) han retornat 400 amb el missatge esperat.
...
**Causa**: la introducció de la data d'ancoratge per compte (entrada anterior "el motor projecta des de l'últim moviment importat...") reutilitzava aquesta mateixa data d'ancoratge (`avui`) per calcular `dataLimit = avui + horitzoDies` a `projectaEsdeveniments` — barrejant dues coses diferents: quan es considera "vençut" un compromís (ha de fer-se servir l'ancoratge, per compte) i fins on arriba l'horitzó triat pel selector (ha de comptar-se sempre des d'avui de veritat, independentment de si el compte està o no actualitzat).

**Fix**: nou paràmetre `avuiReal` a `projectaEsdeveniments` (`backend/src/lib/prevision.ts`), usat només per calcular `dataLimit`; l'ancoratge (`avui`) es continua fent servir només per a la lògica de vençuts/posada al dia. `calculaPrevisio` (`backend/src/db/operations.ts`) calcula `avuiReal = avui ?? isoAvui()` (data real, o l'`avui` explícit dels tests) i el passa a cada crida de `projectaEsdeveniments`, a més d'ampliar els dies totals de la `serieDiaria` (`diesEntre(avuiCompartit, avuiReal) + horitzoDies`) perquè el gràfic arribi exactament fins al mateix límit que la taula d'esdeveniments.

Un test d'`operations.test.ts` que verificava explícitament el comportament ara considerat un bug ("excludes an occurrence beyond horitzoDies counted from the last import date...") s'ha reescrit per confirmar el comportament correcte, més un test nou pel cas contrari (encara s'exclou el que cau fora de l'horitzó comptat des d'avui de veritat), i un test nou a `prevision.test.ts` per a `projectaEsdeveniments` amb `avuiReal` explícit — 229 tests backend en total (abans 227). `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes (frontend sense canvis). Verificat contra una còpia temporal de `dades/finances.db`: el compte ING-CC amb horitzó=30 ara retorna `serieDiaria` fins al 13/08/2026 (abans 07/08/2026); amb tots els comptes i horitzó=90, fins al 12/10/2026 (avui real 14/07/2026 + 90 dies), mantenint el 30/06/2026 (compte més desactualitzat) com a inici del gràfic.

### 2026-07-14 — Bug: un compromís vençut canviava de data (o deixava de ser vençut) segons quins altres comptes hi hagués seleccionats a Previsió

L'usuari va reportar que a la pestanya Previsió, un compromís ja vençut (p. ex. "Manent Casanovas, S.A.", 4.023,25€) canviava de data segons la selecció de comptes activa. Investigant contra una còpia temporal de `dades/finances.db`: amb només el compte BS seleccionat, el compromís (venciment 10/07/2026) es marcava `vençut: true` i es desplaçava al 24/07/2026; amb tots els comptes seleccionats alhora, la data d'ancoratge compartida ("avui" del motor, introduïda la mateixa tarda — vegeu l'entrada "el motor projecta des de l'últim moviment importat...") baixava a 30/06/2026 (perquè un altre compte sense cap relació feia més temps que no s'importava), i el mateix compromís deixava de ser vençut del tot, mostrant-se com un esdeveniment futur a la seva data original.

**Causa**: `calculaPrevisio` (`backend/src/db/operations.ts`) feia servir una única data d'"avui" compartida (la més antiga entre TOTS els comptes seleccionats) per projectar els recurrents de tots els comptes alhora, així que el compromís d'un compte depenia de l'estat d'importació d'un altre compte totalment aliè.

**Fix**: cada compte ara projecta els seus propis recurrents amb la seva pròpia data d'"avui" (`comptes.flatMap(c => projectaEsdeveniments(recurrents.filter(r => r.compteId === c.id), ..., avui ?? calculaAvuiPrevisio([c]), ...))`), mai la compartida entre tots els seleccionats. La data compartida més antiga es continua fent servir només per al punt de partida del **gràfic** (`construeixSerieDiaria`), que necessita un únic dia 0 per a la línia de saldo total combinada — la taula de moviments previstos ja no en depèn. Quan es passa un `avui` explícit (com als tests interns), s'aplica igual a tots els comptes, sense canvis de comportament.

Test invertit a `operations.test.ts` (l'anterior verificava explícitament el comportament ara considerat un bug — reescrit per confirmar que seleccionar un altre compte no canvia el resultat d'un recurrent aliè) — 227 tests backend en total (sense canvis de recompte, només reescriptura). `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes; 36 tests frontend sense canvis (no es va tocar cap fitxer de frontend). Verificat contra una còpia temporal de `dades/finances.db`: "Manent Casanovas, S.A." ara retorna exactament el mateix esdeveniment (`data: 2026-07-24`, `vençut: true`, `dataPrevistaOriginal: 2026-07-10`) tant amb només BS seleccionat com amb tots els comptes.

### 2026-07-14 — Sub-fase 4.4: nova pestanya "Configuració" per als marges/finestres de conciliació

L'usuari va demanar poder configurar els marges d'import i les finestres de dies de la conciliació, agrupats en una nova pestanya "Configuració" on hi hagués "totes les variables que el programa utilitza". Es va preguntar l'abast (via `AskUserQuestion`): només els 4 paràmetres de conciliació, o també la finestra de transferències internes i el nombre de còpies de seguretat conservades — l'usuari va triar incloure-ho tot (6 paràmetres en total), no només els de conciliació.

**Disseny**: nova taula `configuracio` (fila única, migració `008_configuracio.sql`) amb els 6 valors, tots amb els mateixos valors per defecte que abans eren constants fixes: `toleranciaImportConciliacio` (0.15), `finestraConciliacioDies` (3), `diesDesplacamentVencut` (10), `finestraResolucioVencutDies` (30), `diesDiferenciaTransferencies` (2), `maxCopiesSeguretat` (20). Nou mòdul `backend/src/db/configuracio.ts` (`getConfiguracio`/`actualitzaConfiguracio`, amb validació de rang per camp — p. ex. el marge d'import entre 0 i 1, les finestres de dies enters no negatius, mínim 1 còpia de seguretat), re-exportat des de `db/operations.ts`. Nous `GET`/`PATCH /api/configuracio`.

**Fils que ara llegeixen la configuració en lloc de constants fixes**:
- `lib/prevision.ts` es manté una funció **pura** (sense accés a la base de dades, com sempre): s'hi va afegir la interfície `ConfiguracioConciliacio` i un paràmetre `config` (amb `CONFIGURACIO_CONCILIACIO_DEFECTE` per defecte als tests) que `esConciliat`/`esConciliatVencut`/`esdevenimentVencut`/`importCoincideix` reben explícitament; `db/operations.ts` (`calculaPrevisio`) és qui llegeix `getConfiguracio()` i el passa.
- `suggereixTransferencies()` passa `diesDiferenciaTransferencies` a `suggereixTransferenciesInternes` (que ja acceptava un paràmetre opcional amb el mateix valor per defecte).
- `backupDbFile()` (`db/backupFile.ts`) fa servir `getConfiguracio().maxCopiesSeguretat` com a valor per defecte quan no es passa `maxBackups` explícitament (els tests el poden seguir sobreescrivint).

**Frontend**: nova pestanya "Configuració" (`frontend/src/views/Configuracio.tsx`), un camp numèric + botó "Desa" per paràmetre, amb el marge d'import mostrat/editat com a percentatge (0-100) tot i que l'API l'espera com a fracció (0-1). Text de `Maintenance.tsx` sobre "es conserven les 20 més recents" corregit perquè ja no sigui una xifra fixa incorrecta.

Tests nous: `backend/src/db/configuracio.test.ts` (8, get/update/validació de rangs), 5 a `operations.test.ts` (`calculaPrevisio` i `suggereixTransferencies` respecten cadascun dels paràmetres configurats), 1 a `backupFile.test.ts` (`backupDbFile()` sense argument fa servir el `maxCopiesSeguretat` configurat) — 227 tests backend en total. `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes; 36 tests frontend sense canvis. Verificat contra una còpia temporal de `dades/finances.db`: la migració s'aplica neta sobre la base de dades real amb els valors per defecte correctes, `PATCH /api/configuracio` actualitza i persisteix, un valor fora de rang es rebutja amb 400, i `GET /api/previsio` segueix funcionant després del canvi.

### 2026-07-14 — Previsió: el motor projecta des de l'últim moviment importat de cada compte, no des d'avui

L'usuari va demanar que els moviments previstos no es calculin a partir de la data real d'avui sinó a partir de la data de l'últim moviment importat de cada compte, perquè la previsió tingui sentit encara que les dades d'un compte no estiguin actualitzades. Abans de decidir el disseny es van consultar dues qüestions obertes (via `AskUserQuestion`): (1) amb diversos comptes seleccionats amb dates d'última importació diferents, quina data de referència fer servir per a tota la previsió (gràfic + taula, que comparteixen una única línia de temps) — l'usuari va triar **la més antiga entre els seleccionats** (opció recomanada) enlloc de projectar cada compte amb el seu propi horitzó o de restringir el canvi a un sol compte; (2) un compte sense cap moviment importat encara — l'usuari va triar **la data real d'avui** com a valor de reserva (opció recomanada) enlloc de basar-se en el primer recurrent previst.

**Canvi**: nova funció `calculaAvuiPrevisio(comptes)` a `backend/src/db/operations.ts`: per cada compte seleccionat, `MAX(data_operacio)` dels seus moviments (o la data real d'avui si no en té cap); el resultat és la data més antiga (`MIN`) entre tots els comptes. `calculaPrevisio(compteIds, horitzoDies, avui?)` fa servir aquest càlcul com a valor per defecte del paràmetre `avui` (abans `isoAvui()` fix); un `avui` explícit (com el que ja feien servir els tests interns del motor) continua tenint prioritat. La ruta `GET /api/previsio` no passa `avui` explícitament, així que sempre agafa aquest nou comportament en producció. No calen canvis al frontend: `Previsio.tsx` ja era purament una capa de presentació del que retorna el backend.

Tests nous a `operations.test.ts` (ancoratge a la data d'última importació enlloc d'avui real, exclusió d'un venciment fora de l'horitzó comptat des d'aquesta data, reserva a la data real d'avui per a un compte sense moviments, la data més antiga guanya entre diversos comptes seleccionats, un `avui` explícit encara té prioritat) — 213 tests backend en total (`prevision.test.ts` sense canvis, 28 tests). `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes; 36 tests frontend sense canvis. Verificat contra una còpia temporal de `dades/finances.db` (servidor de proves, port 4183, esborrat en acabar): el compte BS (últim moviment importat 2026-07-10, 4 dies abans de la data real 2026-07-14) projecta la previsió amb `serieDiaria[0].data = '2026-07-10'` i un horitzó de 30 dies que acaba el 2026-08-09 (30 dies des de l'ancoratge, no des d'avui).

### 2026-07-14 — Previsió: la columna Import distingeix import aproximat/real, com a Recurrents

L'usuari va demanar que la taula de moviments previstos de Previsió faciliti distingir un import aproximat d'un de real, igual que ja feia la pestanya Recurrents (prefix «≈ » + tooltip). Etiqueta ajustada després a petició de l'usuari: "Import cert" → "**Import real**" (a l'aplicació i a la documentació).

Fins ara `EsdevenimentPrevist` (`backend/src/lib/prevision.ts`) no portava `importAproximat`: el recurrent d'origen sí que el té (des del fix de conciliació del 2026-07-13), però mai es propagava a l'esdeveniment projectat. Afegit el camp a la interfície i als tres punts de `projectaEsdeveniments` on es construeix un esdeveniment (venciment normal d'un `unica`, ocurrència normal d'un periòdic, i `esdevenimentVencut` — que cobreix tant un `unica` vençut com l'última ocurrència vençuda d'un periòdic). Propagat també al tipus equivalent del frontend (`frontend/src/api/types.ts`). A `frontend/src/views/Previsio.tsx`, la cel·la d'Import de la taula ara aplica el mateix tractament que `RecurrentsList.tsx`: prefix «≈ » i `title` «Import aproximat (estimació)» / «Import real».

Tests nous a `prevision.test.ts` (propagació a un esdeveniment normal i a un vençut) — 208 tests backend en total (36 frontend sense canvis). `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes. Verificat contra una còpia temporal de `dades/finances.db` (servidor de proves, port 4182, esborrat en acabar): dels 175 esdeveniments projectats a horitzó d'1 any, els 175 porten el camp `importAproximat` (90 aproximats, 85 reals, cap sense el camp).

### 2026-07-14 — Importació de compromisos (3.2): cada importació substitueix tots els importats anteriors del mateix compte

L'usuari va demanar que, cada vegada que es realitzi una importació de compromisos (factures) d'un compte, s'eliminin tots els compromisos amb `origen='importat'` d'aquest mateix compte que ja hi haguessin, ja que el fitxer d'importació reflecteix l'estat actualitzat de cobraments/pagaments compromesos i només interessa tenir els que actualment estan pendents (un compromís que ja no surt al fitxer nou es considera resolt o retirat).

Abans, `importaRecurrents` (`backend/src/db/operations.ts`) era additiva: dedup només contra ids ja existents d'una importació prèvia del mateix compte (`splitNousRecurrentsIDuplicats`), sense mai eliminar res — un compromís pagat o retirat a l'origen es quedava penjat indefinidament a la previsió fins que algú l'esborrés a mà o es conciliés per casualitat amb un moviment real.

**Canvi**: `importaRecurrents` ara elimina primer tots els `origen='importat'` del compte indicat (dins la mateixa transacció que la inserció), i després insereix els del fitxer. Els recurrents `origen='manual'` i els d'altres comptes no es toquen mai. Dins del mateix fitxer, files coincidents es continuen mantenint com a entrades separades (no es col·lapsen). `ImportaRecurrentsResult` canvia de `{ nous, duplicats }` a `{ nous, eliminats }` (el concepte de "duplicat" entre importacions ja no existeix, ara és sempre reemplaç complet). Actualitzat a `backend/src/db/operations.ts`, `frontend/src/api/types.ts` i `frontend/src/import/RecurrentsImportWizard.tsx` (avís explícit abans de confirmar + resum final amb el nou llenguatge). `especificacio.md` §4.2 documenta el nou comportament.

Tests actualitzats/afegits a `backend/src/db/operations.test.ts`: reemplaç en re-importar el mateix fitxer (`eliminats` reflecteix les files anteriors), eliminació d'un compromís que ja no surt al fitxer nou, no-afectació d'altres comptes ni de recurrents manuals, no-op quan no hi ha res a importar ni eliminar — 207 tests backend en total (`prevision.test.ts` sense canvis, 27 tests). `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes; 36 tests frontend sense canvis (`RecurrentsImportWizard.tsx` no té tests propis, es verifica manualment com la resta de `import/`).

### 2026-07-14 — Fase 6 (opcional, ajornada): integració amb MS Access via notificació HTTP

L'usuari va preguntar si GesFam podria llançar consultes SQL contra una base de dades d'Access per substituir la importació actual. Es va desaconsellar (dependència nativa Windows-only vía ODBC, esquema/fitxer d'Access en viu, risc de bloqueig si Access el té obert). L'usuari va proposar la direcció inversa: que sigui Access qui notifiqui GesFam quan es desa un canvi de pagaments/cobraments. Idea acordada: event `AfterUpdate` en VBA crida un endpoint HTTP de l'API de GesFam (p. ex. `POST /api/moviments/importa-un`) amb `MSXML2.XMLHTTP`, mai escriptura directa sobre el fitxer SQLite (dos processos escrivint el mateix fitxer és fràgil). Afegida com a **Fase 6** a `especificacio.md` §6. **Explícitament posposada per l'usuari** ("ho afrontaré més endevant") — no s'ha dissenyat l'endpoint ni identificat els events/formularis concrets d'Access; no iniciar aquesta fase sense que l'usuari ho demani.

### 2026-07-13 — Bug: la conciliació de previsió podia amagar un compromís vençut per pura coincidència d'import

L'usuari va reportar que un compromís `unica` real i vençut (BS, "Géneros de Punto Fabrés, S.A.U.", 309,18€, venciment 30/06/2026) no apareixia enlloc a la previsió. Investigant contra una còpia temporal de `dades/finances.db`, es va trobar que **8 dels 12 compromisos `unica` vençuts del compte BS desapareixien silenciosament** (no només l'exemple reportat), tots amb la mateixa causa.

**Causa arrel**: `esConciliatVencut`/`esConciliat` (`backend/src/lib/prevision.ts`) només comprovaven compte + mateix signe + import similar (±15%) + data dins la finestra — mai el concepte ni la categoria. El compte BS registra molts ingressos de clients (categoria "AVG-Clients") d'imports similars: el compromís de 309,18€ conciliava falsament contra un ingrés real de 331,06€ d'un client totalment diferent, arribat 3 dies després del venciment i dins del marge del 15%, purament per coincidència.

**Fix acordat amb l'usuari** (dues condicions noves, totes dues obligatòries a `esConciliat` i `esConciliatVencut`):
- **Import**: si el recurrent NO té `importAproximat`, el moviment real ha de tenir l'import EXACTAMENT igual (`importCoincideix`); el marge del 15% (`TOLERANCIA_IMPORT_CONCILIACIO`) només s'aplica quan `importAproximat === true`.
- **Categoria**: el moviment real ha de tenir la mateixa `categoriaId` que el recurrent (inclòs el cas que cap dels dos en tingui).

Canvis: `RecurrentPerProjeccio` guanya `importAproximat: boolean`; `MovimentPerConciliacio` guanya `categoriaId?: string`; `calculaPrevisio` (`db/operations.ts`) passa tots dos camps. `esConciliat`/`esConciliatVencut` passen a rebre el recurrent sencer (abans rebien `compteId`/`importCents` esparsos) per poder consultar `importAproximat`/`categoriaId`.

Tests: 5 de nous a `prevision.test.ts` (exigeix import exacte quan no és aproximat — regressió directa del cas real —, admet marge quan sí ho és, exigeix mateixa categoria, conciliat quan la categoria coincideix, i un vençut no es resol per un import "semblant" no exacte) — 205 tests backend en total (`operations.test.ts` sense canvis, 72 tests). `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes; 36 tests frontend sense canvis (cap fitxer de frontend tocat).

Validat contra l'històric real (còpia temporal, esborrada en acabar): dels 12 compromisos `unica` vençuts del compte BS, els 8 que abans desapareixien ara es mostren correctament com a vençuts, inclòs l'exemple reportat (309,18€, 30/06/2026) — 0 desapareguts després del fix.

`especificacio.md` actualitzat (4.2): la conciliació ara exigeix import exacte (llevat que sigui aproximat) i mateixa categoria.
...
L'usuari va plantejar un cas concret: un càrrec mensual esperat el 15/07, si el dia 20 encara no ha arribat, la previsió no en diu res enlloc (l'ocurrència passada simplement es descarta en avançar al període següent, sense cap avís), i si finalment arriba el dia 22 amb un compte just, la previsió no ho havia anticipat. Revisant el codi per proposar la solució, es va detectar un segon problema relacionat: la finestra de conciliació (±3 dies) mai hauria reconegut aquest pagament com el mateix un cop arribés amb tants dies de retard, així que l'avís de "vençut" es quedaria mostrant-se per sempre encara que el pagament ja hagués arribat de veritat. Es va proposar un pla de dos punts i es va confirmar amb l'usuari (límit de 30 dies per al segon punt).

- **`lib/prevision.ts`**: el tractament de "vençut" (fins ara només per a `unica`) s'estén als recurrents **periòdics**. En avançar un recurrent periòdic amb `dataPrevista` desfasada, el motor ja no descarta silenciosament totes les ocurrències passades: es queda amb **la més recent** (l'immediatament anterior a la propera ocurrència futura) i, si tampoc s'ha conciliat, la projecta com a "vençuda" (mateix tractament visual que un `unica`: desplaçada `DIES_DESPLACAMENT_VENCUT` dies, amb `dataPrevistaOriginal`) — **sense interrompre** la projecció normal de les properes ocurrències futures. Deliberadament només es vigila l'ocurrència més recent (no totes les passades): un recurrent abandonat fa mesos no ha d'omplir la previsió d'avisos repetits.
  - Nova `esConciliatVencut`: com `esConciliat`, però només compta un moviment real des de la data de venciment original **endavant** (mai abans), amb una finestra de **`FINESTRA_RESOLUCIO_VENCUT_DIES` = 30 dies** en lloc dels ±3 dies estrictes — perquè un pagament amb una setmana (o més, fins a 30 dies) de retard es reconegui igualment com a resolt. S'aplica tant al nou cas periòdic com a l'`unica` vençut ja existent (mateix bug, mateixa correcció).
  - La finestra estricta (±3 dies) es manté sense canvis per a qualsevol ocurrència que encara no s'hagi considerat vençuda.
- **`frontend/src/views/Previsio.tsx`**: es va detectar (i corregir) un bug d'interacció abans que arribés a producció: el botó "Elimina" cridava `eliminaOcurrenciaPrevista(recurrentId, e.data)`, però per a una fila vençuda `e.data` és la data **desplaçada** (avui+10), no la data real de venciment — descartar-la hauria avançat `dataPrevista` a partir d'una data incorrecta, desquadrant tot el cicle futur del recurrent. Corregit a `e.dataPrevistaOriginal ?? e.data`.

Tests: reescrits/ampliats a `prevision.test.ts` — el test de "fast-forward silenciós" es divideix en dos (amb l'ocurrència més recent conciliada -> silenciós de veritat; sense conciliar -> vençut, sense aturar la projecció futura), més tests nous per a la finestra de 30 dies (resol un pagament 10 dies tard, no resol un de 35 dies tard, no resol un pagament anterior al venciment) i per confirmar que un periòdic no desfasat no mostra mai cap vençut — 198 tests backend en total (`operations.test.ts` sense canvis, 70 tests). `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes; 36 tests frontend sense canvis.

Validat contra l'històric real (només lectura, còpia temporal esborrada en acabar): la previsió a 90 dies mostra ara 4 factures reals (compromisos `unica` importats) marcades correctament com a vençudes i no conciliades — cap recurrent periòdic real n'és afectat perquè actualment no n'hi ha cap de confirmat a les dades de l'usuari.

`especificacio.md` actualitzat: la secció de conciliació (4.2) explica el tractament de "vençut" i la finestra ampliada de 30 dies per resoldre'l.
...
L'usuari va revisar el comportament dels compromisos puntuals vençuts (introduït en una entrada anterior) i va demanar un canvi: en lloc de projectar-los exactament **avui**, tots s'han d'ubicar a una data **10 dies posterior a avui**, mantenint el color diferenciat ja existent i el text "vençut", i afegint-hi la data de venciment original. Es va preguntar explícitament l'abast abans d'implementar: **només afecta els compromisos puntuals (`unica`)**, no els recurrents periòdics (que ja avancen sols a la seva propera ocurrència real i mai es marquen com a vençuts).

- `lib/prevision.ts`: nova constant `DIES_DESPLACAMENT_VENCUT = 10`. Un `unica` vençut i no conciliat ara es projecta a `avui + 10 dies` (abans: exactament avui), i el nou camp `EsdevenimentPrevist.dataPrevistaOriginal` porta la data de venciment real perquè la UI la pugui mostrar. La comprovació de conciliació **segueix ancorada a la `dataPrevista` original** (no canvia): si canviés a la data desplaçada, un pagament real fet a prop del venciment real però lluny d'avui+10 deixaria de detectar-se.
- `views/Previsio.tsx`/`api/types.ts`: el text "⚠ vençut" ara inclou `(venciment: dd/mm/aaaa)` amb la data original quan `dataPrevistaOriginal` hi és present.

Tests: 2 nous/actualitzats a `prevision.test.ts` (data desplaçada a avui+10 amb `dataPrevistaOriginal` inclòs; un `dataFi` que cau entre avui i avui+10 segueix descartant l'esdeveniment) — 189 tests backend en total. `tsc -b`/`oxlint`/`vite build` nets a totes dues bandes; 36 tests frontend sense canvis.

Sense validació amb dades reals: l'usuari no té actualment cap compromís puntual vençut, així que aquest camí només es verifica amb tests unitaris determinístics (mateixa limitació que l'entrada anterior).
