...
Backend: 179 tests (Vitest) — parsers (inclou `recurrentsFile.ts`, el parser del format fix de compromisos), dedup (inclou `dedup/recurrents.ts`), `lib/` (inclou `liquidacioTargeta.ts`, `computeContrapartidaId`, `computeRecurrentHash`, `normalizeConceptForRecurrence`, i el motor `recurrenceDetection.ts`), `db/operations.ts` contra SQLite en memòria (`GESFAM_DB_PATH=':memory:'`, inclou el mecanisme de liquidació de targeta: marcatge, quadratura, idempotència, cascada via `undoLot`; i el model de recurrents: creació manual, validacions, eliminació, importació amb dedup i resolució de categoria, còpia de seguretat, manteniment), `db/client.ts` (migracions) i `db/backupFile.ts` (còpia + retenció). Frontend: 33 tests — `lib/balance.ts`, `lib/summary.ts`, `lib/dates.ts`, `lib/numbers.ts` (només lògica de visualització; els components de vista, inclosos `RecurrentsImportWizard`/`RecurrentsList`, no tenen tests propis — es verifiquen manualment, seguint el mateix criteri que la resta de `views/`/`import/`). `npx tsc -b` net a totes dues bandes, `npm run build` (frontend) i `oxlint` (frontend) nets.
...
- **Fase 3 (recurrents) iniciada**: l'usuari ha confirmat començar-la, per sub-fases. Pla acordat i documentat a `especificacio.md` (§4.1, §4.2, §6 punt 3): **3.1 model de dades unificat — FET**, **3.2 importació de compromisos confirmats — FET**, **3.3 motor de detecció de periodicitat — FET** (vegeu historial); 3.4 pantalla de revisió/confirmació unificada, 3.5 exclusions i cas de targetes (la resta d'exclusions ja incorporades a la 3.3, veure entrada), 3.6 conciliació (disseny només, frontera amb Fase 4) — encara no implementades.
- **La 3.3 (motor de detecció) és backend-only, sense UI ni persistència**: `GET /api/recurrents/candidats` exposa els candidats recalculats a cada crida (cap taula nova), útil per a verificació i perquè la 3.4 el consumeixi directament. La pantalla de revisió (confirmar/corregir/ignorar un candidat, i persistir aquesta decisió a `recurrents`) és tota de la 3.4.
...
- **Verificació de la UI de 3.2 (`RecurrentsImportWizard`/`RecurrentsList`) només via API/build, no clic a clic**: no hi ha eina de navegador disponible en aquesta sessió; `tsc -b`, `oxlint` i `vite build` nets, i el flux sencer (previsualitzar + confirmar + dedup + resolució de categoria) verificat per HTTP amb un `.xlsx` real generat amb la mateixa llibreria `xlsx`, contra un servidor i dades temporals. Falta la confirmació visual de l'usuari a un navegador real.
