...
- **Un recurrent ja confirmat amb `dataPrevista` passada no s'actualitza automàticament a la seva pròpia fila** (a diferència d'un candidat detectat, que ara sempre es recalcula cap al futur cada vegada que es demana): l'usuari l'ha de corregir a mà des de l'edició in-line si vol que `dataPrevista` reflecteixi la realitat. El motor de previsió (Fase 4, 4.1) ja ho gestiona sense necessitat d'aquesta correcció manual: per a un periòdic avança silenciosament fins la primera ocurrència futura, i per a un `unica` vençut i encara no conciliat, el projecta avui mateix marcat `vençut: true` (vegeu entrada d'historial corresponent) en lloc de desaparèixer o quedar-se enrere.
