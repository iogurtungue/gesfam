...
import { commitImport, createCompte, findMatchingCompte, importFile, previsualitzaManual } from '../api/client';
import type { ColumnMapping, Compte, ParseResult, RawTable } from '../api/types';
  ...
  async function handleFiles(fileList: FileList | null) {
    ...
    for (const file of files) {
      ...
      if (outcome.status === 'parsed') {
        ...
        newItems.push({ key: `${file.name}-map-${Date.now()}`, fileName: file.name, kind: 'needsMapping', file, table: outcome.table });
      } else {
  ...
  if (item.kind === 'needsMapping') {
    return (
      <div style={{ marginBottom: 12 }}>
        ...
        <ManualMapping
          table={item.table}
          onSubmit={async (mapping: ColumnMapping) => {
            const outcome = await previsualitzaManual(item.file, mapping);
            if (outcome.status === 'parsed') {
              onResolved(outcome.results[0]);
            }
          }}
        />
      </div>
